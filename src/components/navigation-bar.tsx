    ...
    <header className="fixed top-0 left-0 w-full z-50 backdrop-blur-lg bg-dark/80 border-b border-white/10">
      <div className="container mx-auto flex items-center justify-between py-4 px-4 md:px-8">
        <div className="flex items-center">
          <Link to="/" className="text-2xl font-display font-semibold text-gradient-gold">
            AestheticsRebellion
          </Link>
        </div>
        ...
        <nav className="flex flex-col items-center justify-center h-full space-y-6 pb-20">
          {navItems.map((item) => (
            <Link
              key={item.name}
              to={item.path}
              className="text-xl font-display text-white/80 hover:text-white transition-colors duration-200"
